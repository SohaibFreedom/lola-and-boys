...
var videoSection = {
  ...
  createFrame: function(settings){
    ...
    if(settings.provider === 'youtube') {
      ...
      html = '<video '+ controls + autoplay + autoloop + poster +' id="video"  muted ><source src="'+settings.videoMP4+'" type="video/mp4"><source src="'+settings.videoOGV+'" type="video/ogg" ></video>';
    }
    return html;
  },
...
//           if (BOLD && BOLD.options && BOLD.options.app && typeof BOLD.options.app.init === "function"){
...
//           }          
  ...
  function ajaxUpdateCart(lineID, quantity, parent, itemKey) {
    $('form[action^="/checkout"]').attr("action","/cart")
    $.ajax({
      url: '/cart/change.js',
      dataType: 'json',
      cache: false,
      ...
      success: function (data) {
        ...
        const itemFromKey = data.items.find((item) => item.key === itemKey);
        if (typeof itemFromKey === "undefined"){
          var updated_quantity = 0;
        } else {
          var updated_quantity = itemFromKey.quantity;
        }
        ...
        if(quantity > 0 && quantity != updated_quantity) {
          ...
          parent.find("input[data-item-key='" + itemKey + "']").parent().after(warning);
          parent.find("input[data-item-key='" + itemKey + "']").val(updated_quantity);
  ...
  function refreshCart(cart) {
    ...
    if (cart.item_count == 0) {
      ...
      let savedby = cart.items.find((item) => item.vendor === "SavedBy");
      let cartTotalPrice = cart.total_price;
      if(savedby) {
        cartTotalPrice = cartTotalPrice - savedby.final_price;
      }
      
      cart_action_html += '<span class="right"><span class="money">' + Shopify.formatMoney(cartTotalPrice, $cart.data('money-format')) + '</span></span>' +
          '<span>Subtotal</span>';
  ...
  $('body').on("change", "#cart_form input.quantity", function() {
    ajaxUpdateCart($(this).data('line-id'), $(this).val(), $(this).parents('#cart_form'), $(this).data('item-key'));
  });

  $('body').on("change", ".cart_content input.quantity", function() {
    ajaxUpdateCart($(this).data('line-id'), $(this).val(), $(this).parents('.cart_content'), $(this).data('item-key'));
  });
...
jQuery(document).ready(function() {
  jQuery(".mobile_nav.dropdown_link").click(function() {
    jQuery('.dropdown_container[data-dropdown="menu"]').toggleClass('dropdown_open');
  });
  jQuery('body').click(function(e) {
    if (!$(e.target).closest('.dropdown_container[data-dropdown="menu"]').length){
        jQuery('.dropdown_container[data-dropdown="menu"]').removeClass('dropdown_open');
    }
 });
});
